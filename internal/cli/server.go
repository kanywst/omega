...
func newServerCommand() *cobra.Command {
	...
	cmd.Flags().StringVar(&identitySourceWLAPI, "identity-source-workload-api", "",
		"SPIFFE Workload API socket to follow the upstream trust bundle from, e.g. unix:///run/spire/sockets/agent.sock. The live alternative to --identity-source-bundle when --identity-source=spire-upstream: the upstream pushes its current anchors and every later rotation, so no file has to be kept in sync and no SIGHUP is involved. Must be a unix:// address - a network endpoint is dialled without transport security, and omega must be an attested workload of the endpoint it takes its trust anchors from.")
