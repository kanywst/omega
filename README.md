...
- **Pluggable identity source**: issue your own SVIDs (built-in CA; the self-signed disk default is dev/eval-only) or **consume identities from an upstream SPIRE / Istio trust domain** (`--identity-source=spire-upstream`) — in upstream mode Omega runs no CA, serves the upstream trust bundle, and applies the authz + audit layer over upstream-issued SVIDs (X.509 today; upstream JWT-SVID validation tracked)
