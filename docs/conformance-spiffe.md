...
| 6 | Sequence / refresh hints | implemented | `GET /v1/spiffe-bundle` emits both `spiffe_sequence` and `spiffe_refresh_hint` (configurable via `--spiffe-bundle-refresh-hint`, default 300s). The sequence tracks the identity source's trust-material generation and increments on every reload that actually changed the material - a SIGHUP re-read of the bundle files ([ADR 0009](adr/0009-upstream-trust-material-reload.md)) or an update pushed over the upstream's SPIFFE Workload API ([ADR 0010](adr/0010-live-upstream-trust-material.md)); it stays at `1` for a built-in issuing source, which cannot rotate its material mid-process. The federation pump in `internal/server/federation` consumes peer-supplied refresh hints to drive the effective per-round poll interval (`min(--ha-poll, min-peer-hint)`, clamped to `[10s, 1h]`) |
